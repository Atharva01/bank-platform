...
def account(auth_headers):
    response = client.post("/api/accounts", json={"owner_name": "REST Test", "balance": 100}, headers=auth_headers)
...
def test_create_and_get_account(account, auth_headers):
    response = client.get(f"/api/accounts/{account}", headers=auth_headers)
...
def test_get_nonexistent_account_returns_404(auth_headers):
    response = client.get("/api/accounts/does-not-exist", headers=auth_headers)
...
def test_list_my_accounts_only_returns_own_accounts(account, auth_headers):
    ...
    try:
        mine = client.get("/api/accounts", headers=auth_headers)
        assert mine.status_code == 200
        assert account in [a["id"] for a in mine.json()]

        theirs = client.get("/api/accounts", headers=other_headers)
...
def test_customer_cannot_access_another_customers_account(account):
    ...
    try:
        response = client.get(f"/api/accounts/{account}", headers=other_headers)
        assert response.status_code == 404
    finally:
        db = SessionLocal()
        row = db.get(Customer, other_id)
...
def test_account_endpoints_require_authentication(account):
    assert client.get(f"/api/accounts/{account}").status_code == 401
    assert client.post("/api/accounts", json={"owner_name": "No Auth"}).status_code == 401


def test_create_and_list_transactions(account, auth_headers):
    create_response = client.post(
        f"/api/accounts/{account}/transactions", json={"amount": 30, "description": "deposit"}, headers=auth_headers
    )
    assert create_response.status_code == 201
    assert create_response.json()["amount"] == 30

    list_response = client.get(f"/api/accounts/{account}/transactions", headers=auth_headers)
    assert list_response.status_code == 200
    assert len(list_response.json()) == 1

    updated_account = client.get(f"/api/accounts/{account}", headers=auth_headers)
    assert updated_account.json()["balance"] == 130


def test_overdraft_transaction_returns_422(account, auth_headers):
    response = client.post(f"/api/accounts/{account}/transactions", json={"amount": -500}, headers=auth_headers)
...
def test_create_and_get_service_request(account, auth_headers):
    create_response = client.post(
        f"/api/accounts/{account}/service-requests", json={"request_type": "change_of_address"}, headers=auth_headers
    )
    assert create_response.status_code == 201
    request_id = create_response.json()["id"]
    assert create_response.json()["status"] == "pending"

    get_response = client.get(f"/api/service-requests/{request_id}", headers=auth_headers)
...
def test_delete_account_cascades_to_transactions_and_service_requests(account, auth_headers, staff_token):
    txn_response = client.post(f"/api/accounts/{account}/transactions", json={"amount": 30}, headers=auth_headers)
    ...
    delete_response = client.delete(f"/api/accounts/{account}", headers={"Authorization": f"Bearer {staff_token}"})
    assert delete_response.status_code == 200

    assert client.get(f"/api/accounts/{account}", headers=auth_headers).status_code == 404
    assert client.get(f"/api/transactions/{transaction_id}", headers=auth_headers).status_code == 404
    assert client.get(f"/api/service-requests/{request_id}", headers=auth_headers).status_code == 404
