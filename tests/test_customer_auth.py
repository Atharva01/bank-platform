...
def reset_rate_limiter():
    # /customers/register and /customers/login are rate-limited (5/min,
    # same as /auth/login) - reset so this file's own repeated calls never
...
def test_register_creates_a_customer_and_returns_a_token():
    ...
    try:
        response = client.post("/customers/register", json={"username": username, "password": "a-real-password"})
...
def test_register_rejects_a_short_password():
    response = client.post(
        "/customers/register", json={"username": f"short-pw-{uuid.uuid4().hex[:8]}", "password": "short"}
    )
    assert response.status_code == 400


def test_register_rejects_a_duplicate_username(customer_user):
    response = client.post("/customers/register", json={"username": _TEST_USERNAME, "password": "irrelevant123"})
    assert response.status_code == 409


def test_login_with_correct_credentials_returns_a_token(customer_user):
    response = client.post("/customers/login", data={"username": _TEST_USERNAME, "password": _TEST_PASSWORD})
...
def test_login_with_wrong_password_returns_401(customer_user):
    response = client.post("/customers/login", data={"username": _TEST_USERNAME, "password": "wrong"})
    assert response.status_code == 401


def test_login_with_unknown_username_returns_401():
    response = client.post("/customers/login", data={"username": "nobody", "password": "irrelevant"})
    assert response.status_code == 401
